    ...
    switch (field) {
      case "name":
        if (typeof value === "string") {
          ...
          (
            this as unknown as { updateNameCapsule: () => void }
          ).updateNameCapsule();
        }
        break;
      ...
      case "anim":
        if (typeof value === "string") {
          const isSit = value.includes("_sit_");
          this.isRemoteSitting = isSit;
...
export {};
(
  Phaser.GameObjects.GameObjectFactory as unknown as {
    register: (
      key: string,
      factory: (
